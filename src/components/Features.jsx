...
// Utility function (replace with your actual one if needed)
...
const BentoGridItem = ({ title, description, icon, className, size = 'small' }) => {
  ...
  return (
    <motion.div
      variants={variants}
      className={cn(
        'group border-gray-600 bg-black hover:border-primary/30 relative flex h-full cursor-pointer flex-col justify-between overflow-hidden rounded-xl border px-6 pt-6 pb-10 shadow-md transition-all duration-500',
        className
      )}
    >
      ...
      <div className="text-red-700 group-hover:text-white absolute right-1 bottom-3 scale-[6] transition-all duration-700 group-hover:scale-[6.2]">
        {icon}
      </div>

      <div className="relative z-10 flex h-full flex-col justify-between">
        <div>
          <div className="bg-primary/10 text-red-600 shadow-primary/10 group-hover:bg-primary/20 group-hover:shadow-primary/20 mb-4 flex h-12 w-12 items-center justify-center rounded-full shadow transition-all duration-500">
            {icon}
          </div>
          <h3 className="mb-2 text-xl font-semibold tracking-tight text-white">{title}</h3>
          <p className="text-muted-foreground text-sm">{description}</p>
        </div>
        <div className="text-white mt-4 flex items-center text-sm">
